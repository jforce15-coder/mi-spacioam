...
.pya-head-sub { font-family: var(--sans); font-size: 12.5px; letter-spacing: 0.04em; line-height: 1.6; color: var(--earth); margin: 8px 0 0; max-width: 560px; text-wrap: pretty; }
...
.pya-drop-hint { font-family: var(--sans); font-size: 11px; letter-spacing: 0.03em; line-height: 1.5; color: var(--earth); margin-top: 4px; text-wrap: pretty; }
...
.pya-stat span { font-family: var(--sans); font-size: 10px; font-weight: 500; letter-spacing: 0.14em; text-transform: uppercase; color: var(--earth); }
...
.pya-fchip { border: 1px solid var(--ink-08); background: var(--alabaster); cursor: pointer; border-radius: 999px; padding: 7px 13px; font-family: var(--sans); font-size: 10.5px; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; color: var(--earth); transition: all .16s var(--ease); }
...
.pya-table th { position: sticky; top: 0; z-index: 2; background: var(--beige-soft); font-family: var(--sans); font-size: 9px; font-weight: 600; letter-spacing: 0.14em; text-transform: uppercase; color: var(--earth); text-align: left; padding: 12px 12px; white-space: nowrap; }
...
.pya-badge.sin { background: var(--beige-soft); color: var(--earth); }
...
.pya-mini-btn.empty { color: var(--earth); }
...
.pya-btn-ghost { background: transparent; color: var(--earth); border: 1px solid var(--ink-08); }
...
.pya-kv dt { font-family: var(--sans); font-size: 10px; font-weight: 500; letter-spacing: 0.16em; text-transform: uppercase; color: var(--earth); }
...
.pya-copy { border: none; background: transparent; cursor: pointer; color: var(--earth); display: inline-flex; align-items: center; gap: 5px; font-family: var(--sans); font-size: 10px; letter-spacing: 0.06em; padding: 2px 0; }
...
.pya-empty { padding: 36px 24px; text-align: center; font-family: var(--sans); font-size: 12.5px; letter-spacing: 0.03em; color: var(--earth); }
...
.pya-mode { display: inline-flex; align-items: center; gap: 8px; border: none; cursor: pointer; background: transparent; border-radius: 11px; padding: 10px 16px; font-family: var(--sans); font-size: 11px; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; color: var(--earth); transition: all .16s var(--ease); }
...
.pya-field > label { font-family: var(--sans); font-size: 9.5px; font-weight: 600; letter-spacing: 0.16em; text-transform: uppercase; color: var(--earth); }
...
.pya-segbtn button { border: none; cursor: pointer; background: transparent; border-radius: 8px; padding: 8px 14px; font-family: var(--sans); font-size: 11px; letter-spacing: 0.04em; color: var(--earth); }
...
.pya-dep-ocr { font-family: var(--sans); font-size: 10px; letter-spacing: 0.06em; color: var(--earth); display: inline-flex; align-items: center; gap: 5px; }
...
.pya-copyfield-lbl { display: block; font-family: var(--sans); font-size: 9px; font-weight: 600; letter-spacing: 0.14em; text-transform: uppercase; color: var(--earth); margin-bottom: 4px; }
...
.pya-copyfield-btn { flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border: none; background: var(--beige-soft); border-radius: 8px; cursor: pointer; color: var(--earth); transition: all .14s var(--ease); }
...
.pya-cal-dow span { text-align: center; font-family: var(--sans); font-size: 9.5px; font-weight: 600; letter-spacing: 0.04em; color: var(--earth); padding: 4px 0 7px; }
    ...
    <div className="pya-mini" ref={ref}>
      ...
      {open && (
        <div className="pya-mini-pop">
          ...
          {!filtered.length && <div style={{ padding: "10px 12px", fontFamily: "var(--sans)", fontSize: 11.5, color: "var(--earth)" }}>Sin resultados</div>}
        </div>
      )}
    </div>
  );
    ...
    <div className="pya-overlay" onClick={onClose}>
      <div className="pya-modal" onClick={e => e.stopPropagation()}>
        <div className="pya-modal-head">
          <div>
            <div style={{ fontFamily: "var(--sans)", fontSize: 10, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--earth)" }}>{lang === "es" ? "Pedido PedidosYa" : "PedidosYa order"}</div>
        ...
        <div className="pya-modal-body">
          <dl style={{ margin: 0 }}>
            ...
            {o.amountNoDiscount > 0 && o.amountNoDiscount !== o.amount && <div className="pya-kv"><dt>{lang === "es" ? "Sin descuento" : "No discount"}</dt><dd style={{ color: "var(--earth)" }}>{P.money(o.amountNoDiscount)}</dd></div>}
            ...
            <div className="pya-items">
              {o.items.map((it, i) => (
                <div className="pya-item" key={i}>
                  ...
                  <span className="pya-num" style={{ color: "var(--earth)" }}>{P.money(it.amount)}</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="pya-overlay" onClick={onClose}>
      <div className="pya-modal" onClick={e => e.stopPropagation()}>
        <div className="pya-modal-head">
          <div>
            <div style={{ fontFamily: "var(--sans)", fontSize: 10, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--earth)" }}>{lang === "es" ? "Factura(s) SAT" : "SAT invoice(s)"}</div>
            <div style={{ fontFamily: "var(--serif)", fontSize: 24, color: "var(--ink)", marginTop: 5, lineHeight: 1.1 }}>{row.order.vendor || "—"}</div>
          </div>
          <button className="pya-modal-x" onClick={onClose}><Icon name="x" size={17} stroke="var(--ink)" /></button>
        </div>
        <div className="pya-modal-body">
          {!invs.length && <p style={{ fontFamily: "var(--sans)", fontSize: 12.5, color: "var(--earth)", margin: "0 0 14px" }}>{lang === "es" ? "Este pedido aún no tiene factura cruzada en el Excel SAT." : "This order has no SAT invoice matched yet."}</p>}
          {invs.map((inv, i) => (
  ...
  return (
    <div className="pya-overlay" onClick={onClose}>
      <div className="pya-modal" style={{ maxWidth: 780 }} onClick={e => e.stopPropagation()}>
        <div className="pya-modal-head">
          <div>
            <div style={{ fontFamily: "var(--sans)", fontSize: 10, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--earth)" }}>{es ? "Factura del pedido" : "Order invoice"}</div>
            ...
            <div style={{ fontFamily: "var(--sans)", fontSize: 11.5, color: "var(--earth)", marginTop: 4 }}>
        ...
        <div className="pya-modal-body">
          ...
          {tab === "order" && !data.orderUrl && (
            <div className="pya-frame-fallback" style={{ marginBottom: 12 }}>
              ...
              <p style={{ fontFamily: "var(--sans)", fontSize: 12.5, lineHeight: 1.55, color: "var(--earth)", margin: 0, maxWidth: 360 }}>{es ? "Aún no se ha pegado la URL del pedido para esta factura." : "No order URL has been pasted for this invoice yet."}</p>
            </div>
          )}
  ...
  return (
    <div className="pya-datewrap" ref={ref}>
      <button type="button" className="pya-input pya-datebtn" onClick={() => setOpen(o => !o)}>
        <Icon name="calendar" size={14} stroke="var(--earth)" />
