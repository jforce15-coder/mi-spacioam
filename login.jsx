...
const Login = ({ lang, setLang, onLogin }) => {
  ...
  const field = (label, value, onChange, ph, type, extra, autoFocus) => (
    <label style={{ display: "flex", flexDirection: "column", gap: 9 }}>
      <span style={{ fontFamily: "var(--sans)", fontSize: 10, fontWeight: 500, letterSpacing: "0.28em", textTransform: "uppercase", color: "var(--fg-muted)" }}>{label}</span>
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        <input value={value} onChange={e => { onChange(e.target.value); setErr(false); }} placeholder={ph} type={type}
          ...
          onFocus={e => { e.target.style.borderColor = "var(--peach)"; e.target.style.boxShadow = "0 0 0 3px rgba(233,130,106,.35)"; }}
          onBlur={e => { e.target.style.borderColor = err ? "var(--peach)" : "var(--warm-grey)"; e.target.style.boxShadow = "none"; }} />
        {extra}
      </div>
    </label>
  );
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "1fr", background: "var(--alabaster)" }} className="sa-login">
      ...
      <aside className="sa-login-aside" style={{
        ...
        <div style={{ position: "relative", zIndex: 2, fontFamily: "var(--sans)", fontSize: 10.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--fg-muted)" }}>
          Guatemala · {t("help")}
        </div>
      </aside>
      ...
      <main style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "40px 24px", position: "relative" }}>
        ...
        <div style={{ width: "100%", maxWidth: 380 }}>
          ...
          <form onSubmit={needPass ? createPass : submit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {err && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "var(--sans)", fontSize: 12, letterSpacing: "0.03em", color: "var(--attention-text)", marginTop: -4 }}>
                <Icon name="info" size={15} /> {t("login_error")}
              </div>
            )}

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 2 }}>
              <span style={{ fontFamily: "var(--sans)", fontSize: 11.5, letterSpacing: "0.04em", color: "var(--fg-muted)" }}>{t("forgot")}</span>
            </div>
